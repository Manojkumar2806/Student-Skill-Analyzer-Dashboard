...
import DSBdashboard from './DSBdashoard';
...
const DashboardB = () => {
  ...
  const fetchSheetData = async () => {
    ...
    try {
      ...
      const normalizedData = data.map((row, index) => {
        ...
        normalizedRow.class = 'CSD-B'; // Hardcode Class
  ...
  return (
    <div className="container mt-4">
      <h2 className="text-center mb-4">Section B</h2>
      ...
      <div className="mt-3">
        ...
        {activeTab === 'table' && (
          <div className="table-responsive">
            <table className="table table-bordered table-striped" aria-label="Section B data table">
              <thead>
                <tr>
                  {keys.map((key) => (
        ...
        {/* Dashboard view */}
        {activeTab === 'dashboard' && <DSBdashboard data={sheetData} />}
      </div>
    </div>
  );
};
