...
const prepareLineChartData = (data, activeStudent) => {
  let filteredData = activeStudent
    ...
    : [...data]
        .filter(student => typeof student.Total === "number" && !isNaN(student.Total))
        .sort((a, b) => b.Total - a.Total)
        .slice(0, 20); // Top 20 if no student selected

  return filteredData.map(student => {
    const java = typeof student["Java-35m"] === "number" ? student["Java-35m"] : 0;
...
const prepareStudentDonutChartData = (data, activeStudent) => {
  ...
  const javaMark = typeof student["Java-35m"] === "number" ? student["Java-35m"] : 0;
  ...
  const mlMark = Math.max(totalMark - (javaMark + pythonMark), 0); // Avoid negatives

  return [
    { name: "Java", value: parseFloat(((javaMark / 35) * 100).toFixed(1)) },
    { name: "Python", value: parseFloat(((pythonMark / 35) * 100).toFixed(1)) },
    { name: "Machine Learning", value: parseFloat(((mlMark / 30) * 100).toFixed(1)) },
